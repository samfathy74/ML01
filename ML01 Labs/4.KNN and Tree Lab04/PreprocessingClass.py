...
# =====================================[check outlier]==========================================
    def detected_outliers(df, threshold=1.5, clipping=False):
        outlier_dict = dict()
        if len(df) > 0:
            for col in df:
                ...
                # clamping outliers
                if clipping == True and outlier_dict[col] != []:
...
# =====================================[separated data]==========================================
    def separate_data(df, cols_only=False):
        numeric = df.select_dtypes(include=np.number).copy()
        categorical = df.select_dtypes(exclude=np.number).copy()
        if cols_only:
            return numeric.columns, categorical.columns
        else:
            return numeric, categorical

#====================================[determine corr]==========================================
    def detected_highly_corr(df, target_name, threshold=0.7):
...
# =====================================[determine skewness]==========================================
    def skewness_calc(num_df, threshold=80, transform=False, method='log'):
        ...
        if transform==True:
            for col_name in col_list:
                match method:
                    case 'log':
                        num_df[col_name] = np.log(num_df[col_name]) 
                        print('|+| Transformed: ', col_name)
                        return num_df
                    case 'sqrt':
                        num_df[col_name] = np.sqrt(num_df[col_name])
                        print('|+| Transformed: ', col_name)
                        return num_df
                    case 'exp':
                        num_df[col_name] = np.exp(num_df[col_name])
                        print('|+| Transformed: ', col_name)
                        return num_df
                    case 'boxcox':
                        num_df[col_name] = boxcox(num_df[col_name])[0]
                        print('|+| Transformed: ', col_name)
                        return num_df
...
# =====================================[Encoding Categorical]==========================================
    def encoding_ordinal_cat(ordinal_cats, df_train, df_test=None):
    ...
    def encoding_nominal_cat(nominal_cats, df_train, df_test=None):
        ...
        cols = one_hot_enc_model.get_feature_names_out(nominal_cats)
        df_train_data = pd.DataFrame(nominal_train_data, columns=cols, index=df_train.index)
        
        nominal_train_df = pd.concat([df_train_data, df_train], axis=1)
        nominal_train_df.drop(nominal_cats, axis=1, inplace=True)
        
        if df_test is None:
            return nominal_train_df
        else:
            df_test_data = pd.DataFrame(nominal_test_data, columns=cols, index=df_test.index)
            nominal_test_df = pd.concat([df_test_data, df_test], axis=1)
            nominal_test_df.drop(nominal_cats, axis=1, inplace=True)

            return nominal_train_df, nominal_test_df 

# =====================================[Scaling]==========================================
    def scaling(X_train, X_test, scaling_type='minmax'):
        col_num = X_train.columns
        if scaling_type == 'standard':
            scaler = StandardScaler()
            scaler_train_data = pd.DataFrame(scaler.fit_transform(X_train), columns = col_num, index=X_train.index)
            scaler_test_data = pd.DataFrame(scaler.transform(X_test), columns = col_num, index=X_test.index)
            return scaler_train_data, scaler_test_data
        elif scaling_type == 'minmax':
            scaler = MinMaxScaler()
            min_max_train_data = pd.DataFrame(scaler.fit_transform(X_train), columns = col_num, index=X_train.index)
            min_max_test_data = pd.DataFrame(scaler.transform(X_test), columns = col_num, index=X_test.index)
            return min_max_train_data, min_max_test_data
        elif scaling_type == 'robust':
            scaler = RobustScaler()
            robust_train_data = pd.DataFrame(scaler.fit_transform(X_train), columns = col_num, index=X_train.index)
            robust_test_data = pd.DataFrame(scaler.transform(X_test), columns = col_num, index=X_test.index)
            return robust_train_data, robust_test_data

        elif scaling_type == 'normal':
            scaler = Normalizer()
            normal_train_data = pd.DataFrame(scaler.fit_transform(X_train), columns = col_num, index=X_train.index)
            normal_test_data = pd.DataFrame(scaler.transform(X_test), columns = col_num, index=X_test.index)
            return normal_train_data, normal_test_data


# =====================================[MultiColinearity]==========================================
    def multi_collinearity(df, threshold=20, drop_cols=False):
