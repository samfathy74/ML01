...
# =====================================[check outlier]==========================================
    def detected_outliers(df, threshold=1.5, clamping=True):
        outlier_dict = dict()
        if len(df) > 0:
            for col in df:
...
# =====================================[histplot]===================================================
    def histplot(df, bins=30):
        ...
        for i in range(0, n):
            plt.subplot(n, round(np.sqrt(n)), i+1)
            histplot(data=df.iloc[:, i], bins=bins)
...
# =====================================[MultiColinearity]==========================================
    ...
    #     return vif_data.sort_values(by="VIF", ascending=False) , vif_data.count()[1]
    ...
    def multi_colinearity(df, threshold=20, drop_cols=False):
        ...
        col_drop_list = []
        for col in vif_data.index:
            if vif_data.loc[col, 'VIF'] > threshold:
